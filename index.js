...
function highToLow(hotel1, hotel2) {
...
app.get('/hotels/sort/pricing', (req, res) => {
  ...
  if (pricing === 'low-to-high') {
    hotelsCopy.sort(lowToHigh);
  } else {
    hotelsCopy.sort(highToLow);
  }
  res.json(hotelsCopy);
});
...
app.get('/hotels/sort/rating', (req, res) => {
  ...
  if (rating === 'low-to-high') {
    hotelsCopy.sort(ratingLowToHigh);
  } else {
    hotelsCopy.sort(ratingHighToLow);
  }
  res.json(hotelsCopy);
});

// Endpoint 3: Get the Hotels sorted based on their Reviews
function reviewsLowToHigh(hotel1, hotel2) {
  return hotel1.reviews - hotel2.reviews;
}
function reviewsHighToLow(hotel1, hotel2) {
...
app.get('/hotels/sort/reviews', (req, res) => {
  ...
  if (reviews === 'least-to-most') {
    hotelsCopy.sort(reviewsLowToHigh);
  } else {
    hotelsCopy.sort(reviewsHighToLow);
  }
  res.json(hotelsCopy);
});
...
app.get('/hotels/filter/amenity', (req, res) => {
  ...
  let updatedHotels = hotels.map((hotel) => ({
    ...hotel,
    amenity: hotel.amenity.toLowerCase(),
  }));
  let result = updatedHotels.filter((hotelObj) =>
...
app.get('/hotels/filter/country', (req, res) => {
  ...
  let updatedHotels = hotels.map((hotel) => ({
    ...hotel,
    country: hotel.country.toLowerCase(),
  }));
  let result = updatedHotels.filter((hotelObj) =>
...
app.get('/hotels/filter/category', (req, res) => {
  ...
  let updatedHotels = hotels.map((hotel) => ({
    ...hotel,
    category: hotel.category.toLowerCase(),
  }));
  let result = updatedHotels.filter((hotelObj) =>
